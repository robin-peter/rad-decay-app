...
radioisotope = st.selectbox(
    "Select a Radioisotope:", options=list(radioisotopes))
a_units = ["uCi", "mCi", "Ci", "Bq", "kBq", "MBq"]
t_units = ["d", "h", "m", "s"]
# factor by which to convert s to the time unit above
t_factors = [(3600 * 24), 3600, 60, 1]
...
with col1:
    Aa0 = st.number_input(f"Initial {nuc_p} activity:", min_value=0.0,
                          max_value=200.0, value=1.0, step=0.1, format="%.3f")
    selected_date = st.date_input(
        "Date/time of initial activity:", datetime.date.today())

with col2:
    Ab0 = st.number_input(f"Initial {nuc_d} activity:", min_value=0.0,
                          max_value=200.0, value=0.0, step=0.1, format="%.3f")
...
df = inv.decay_time_series_pandas(
    time_period=max_value, time_units=t_unit, decay_units=A_unit)
# select only relevant isotopes (from among whole chain)
df = df[[nuc_p, nuc_d]]
